...
import com.zjyk.quality.ui.present.QALoginPresenterImpl;
...
public class QAActivityLogin extends QABaseActivity implements View.OnClickListener, QAILoginView {
...
//    RelativeLayout rtTitle;
    ...
    private QAPerson userBean;
    /**
     * 是否是管理员 true表示是
     */
//    private QAProgressHUD progressHUD;
//    private QALoginPresenterImpl loginPresent;
    ...
    protected void creatPresent() {
        currentPresent = new QALoginPresenterImpl(this);
    }
...
//        loginPresent = new QALoginPresenterImpl(this);
        if (loginType == 0) {
...
//    @VerifyActivityIsFinsh
    public void loginSuccess(QAPerson userBean) {
    ...
    @Override
    public void uploadResult(QAPerson person) {
        if (person != null) {
    ...
    @Override
    public void quitResult(QAPerson person) {
        if (person != null) {
...
//        mCameraRenderThread.quit();
    ...
    private void savePhoto(final Bitmap b) {
        ...
        photoPath = ((QALoginPresenterImpl) currentPresent).savePhoto(userBean, b);
    }
...
//        userBean.setPhoto(photoPath);
        int resultCode;
//        if (!isAdmin) {
//            resultCode = QAConstants.C_LOGIN_REQUEST_USER;
//            workstation.zjyk.line.ui.login.model.UserManager.getInstance().updateUserInfo(userBean);
//        } else {
//            resultCode = QAConstants.C_LOGIN_REQUEST_ADMIN;
...
//        finish();
    ...
    private void requestLogin(String scanResult) {
        ((QALoginPresenterImpl) currentPresent).login(scanResult);
    }

    private void requesQuit(String scanResult) {
        ((QALoginPresenterImpl) currentPresent).quit(scanResult);
    }
}
