package cn.com.ethank.mylibrary.resourcelibrary.toast.custom_toast;
...
public class Toasty {
    ...
    private static int textSize = 35; // in 35SP

    private static boolean tintIcon = true;
    private static Toast currentErrorToast;
    private static Toast currentInfoToast;
    ...
    public static Toast info(@NonNull Context context, @NonNull CharSequence message, int duration, boolean withIcon) {
        return customInfo(context, message, ToastyUtils.getDrawable(context, R.drawable.ic_info_outline_white_48dp),
    ...
    public static Toast custom(@NonNull Context context, @NonNull CharSequence message, Drawable icon,
                               @ColorInt int tintColor, int duration,
                               boolean withIcon, boolean shouldTint) {
        if (currentErrorToast == null) {
            currentErrorToast = new Toast(context);
            currentErrorToast.setGravity(Gravity.CENTER, 0, 0);
            ...
            textSize =context.getResources().getDimensionPixelSize(R.dimen.text_design_35px);
            toastTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, textSize);
//            toastTextView.setTextSize(TypedValue.COMPLEX_UNIT_SP, textSize);
            currentErrorToast.setView(toastLayout);
        }
        TextView toast_text = currentErrorToast.getView().findViewById(R.id.toast_text);
        toast_text.setText(message);
        currentErrorToast.setDuration(duration);
        return currentErrorToast;
    }

    @CheckResult
    public static Toast customInfo(@NonNull Context context, @NonNull CharSequence message, Drawable icon,
                                   @ColorInt int tintColor, int duration,
                                   boolean withIcon, boolean shouldTint) {
        if (currentInfoToast == null) {
            currentInfoToast = new Toast(context);
            currentInfoToast.setGravity(Gravity.CENTER, 0, 0);
            final View toastLayout = ((LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE))
                    .inflate(R.layout.toast_layout, null);
            final ImageView toastIcon = toastLayout.findViewById(R.id.toast_icon);
            final TextView toastTextView = toastLayout.findViewById(R.id.toast_text);
            ...
            toastTextView.setTypeface(currentTypeface);
            toastTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, textSize);
            currentInfoToast.setView(toastLayout);
        }
        TextView toast_text = currentInfoToast.getView().findViewById(R.id.toast_text);
        toast_text.setText(message);
        currentInfoToast.setDuration(duration);
        return currentInfoToast;
    }
