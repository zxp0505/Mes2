...
public class NetworkConnectChangedReceiver extends BroadcastReceiver {
    ...
    public static final String TAG1 = "NetworkConnect";
        ...
        if (WifiManager.WIFI_STATE_CHANGED_ACTION.equals(intent.getAction())) {
            ...
            switch (wifiState) {
                case WifiManager.WIFI_STATE_DISABLED:
                    NetStatusUtil.getInstance().setEnableWifi(false);
                    break;
                ...
                case WifiManager.WIFI_STATE_ENABLED:
                    NetStatusUtil.getInstance().setEnableWifi(true);
                    break;
            ...
            if (activeNetwork != null) { // connected to the internet
                if (activeNetwork.isConnected()) {
                    if (activeNetwork.getType() == ConnectivityManager.TYPE_WIFI) {
                        // connected to wifi
                        if(!NetStatusUtil.getInstance().isWifi()){
                            NetStatusUtil.getInstance().setWifi(true);
                            EventBus.getDefault().post(new MessageEventBean(1,3));
                        }
                        Log.e(TAG, "当前WiFi连接可用 ");
                    } else if (activeNetwork.getType() == ConnectivityManager.TYPE_MOBILE) {
                        // connected to the mobile provider's data plan
                        NetStatusUtil.getInstance().setMobile(true);
...
//                NetStatusUtil.getInstance().setWifi(false);
//                NetStatusUtil.getInstance().setMobile(false);
//                NetStatusUtil.getInstance().setConnect(false);
