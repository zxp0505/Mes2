...
import workstation.zjyk.workstation.ui.present.WSLoginPresenterImpl;
...
public class WSActivityLogin extends WSBaseActivity implements View.OnClickListener, ILoginView {
...
//    Toolbar toolbar;
    ...
    private WSPerson userBean;
    /**
     * 是否是管理员 true表示是
     */
//    private WSProgressHUD progressHUD;
//    private WSLoginPresenterImpl loginPresent;
    ...
    protected void creatPresent() {
        currentPresent = new WSLoginPresenterImpl(this);
    }
...
//        loginPresent = new WSLoginPresenterImpl(this);
        if (loginType == 0) {
    ...
    @OnClick({R.id.tv_back, R.id.ll_login, R.id.iv_login_photograph, R.id.btn_login, R.id.tv_title, R.id.tv_hand_barcode})
...
//    @VerifyActivityIsFinsh
    public void loginSuccess(WSPerson userBean) {
    ...
    @Override
    public void uploadResult(WSPerson person) {
        if (person != null) {
    ...
    @Override
    public void quitResult(WSPerson person) {
        if (person != null) {
...
//        mCameraRenderThread.quit();
    ...
    private void savePhoto(final Bitmap b) {
        ...
        photoPath = ((WSLoginPresenterImpl) currentPresent).savePhoto(userBean, b);
    }
...
//        userBean.setPhoto(photoPath);
        int resultCode;
//        if (!isAdmin) {
//            resultCode = WSConstants.C_LOGIN_REQUEST_USER;
//            workstation.zjyk.line.ui.login.model.UserManager.getInstance().updateUserInfo(userBean);
//        } else {
//            resultCode = WSConstants.C_LOGIN_REQUEST_ADMIN;
...
//        finish();
    ...
    private void requestLogin(String scanResult) {
        ((WSLoginPresenterImpl) currentPresent).login(scanResult);
    }

    private void requesQuit(String scanResult) {
        ((WSLoginPresenterImpl) currentPresent).quit(scanResult);
    }
}
